...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  while( client->time1000 >= 1000 )
  {
    ...
    //"Freeze" if frozen
    if( ent->client->ps.stats[ STAT_HEALTH ] > 0 && ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && ent->frozen >= 1 )
    {
      int       entityList[ MAX_GENTITIES ];
      vec3_t    range = { UNFREEZE_RADIUS, UNFREEZE_RADIUS, UNFREEZE_RADIUS };
      vec3_t    mins, maxs;
      int       i, num;
      ...
      {
        trap_SendServerCommand( -1, "cp \"^1You are Frozen! Ask a teammate for help!\n\"" );
      }
      client->grabExpiryTime = level.time + 1500;	//using grab as freeze method
      client->ps.stats[ STAT_STATE ] = SS_GRABBED;
      G_Damage( ent, NULL, NULL, NULL, NULL, 2, DAMAGE_NO_PROTECTION, MOD_FREEZE );
      ...
      for( i = 0; i < num; i++ )
      {
        teamMember = &g_entities[ entityList[ i ] ];

        if( teamMember->client && teamMember->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS &&
            !(teamMember->frozen)  )
        {
          ent->unfreezeTime += 1000;
          if(ent->unfreezeTime >= TIME_TO_UNFREEZE)
          {
            G_UnfreezeClient(ent);
          }
        }
      }
    }
