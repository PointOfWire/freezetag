...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_freeze, "g_freeze", "1", CVAR_ARCHIVE, 0, qfalse  },
  { &g_freezeTimeLimit, "g_freezeTimeLimit", "6", CVAR_ARCHIVE, 0, qfalse  }
};
...
void CheckExitRules( void )
{
  ...
  if( g_freeze.integer ) //Similar to timelimit so wutever FREEZE
  {
    if( level.time - level.startTime >= g_freezeTimeLimit.integer * 60000 )
    {
