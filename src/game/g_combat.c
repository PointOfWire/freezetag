...
finish_dying: // from MOD_SLAP
  ...
  if ( ( g_freeze.integer >= 1) && self->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && meansOfDeath != MOD_FREEZE) {
    G_FreezeClient(self);
  }
  else {
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if( !( dflags & DAMAGE_NO_PROTECTION ) )
  {
    ...
    if(targ->s.eType == ET_BUILDABLE && targ->biteam == BIT_ALIENS && g_freeze.integer)
      return;
  }
